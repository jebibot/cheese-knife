(() => {
  ...
  const findNestedValue = (value, criteria, depth = 0, seen = new WeakSet()) => {
    ...
    for (const item of values) {
      const found = findNestedValue(item, criteria, depth + 1, seen);
      if (found != null) {
        return found;
      }
    }
  ...
  const getWebpackRequire = new Promise((resolve) => {
    let resolved = false;
    const id = `knife_${Date.now().toString(36)}`;

    const resolveOnce = (req) => {
      if (resolved || typeof req !== "function") {
        return;
      }
      resolved = true;
      resolve(req);
    ...
    const getWebpackChunks = () => {
      return Object.entries(window)
        ...
        .sort(([a], [b]) => {
          const score = (name) =>
            (/glive|chzzk|nng|naver/i.test(name) ? 0 : 10) +
            (/^webpackChunkglive_fe_pc$/.test(name) ? -10 : 0);
          return score(a) - score(b);
        });
    };

    const injectIntoChunk = (chunk) => {
      try {
        chunk.push([
          [id],
          {
            [id]: (module, exports, __webpack_require__) => {
              resolveOnce(__webpack_require__);
            },
          },
          (req) => req(id),
        ]);
    ...
    let tries = 0;
    const probe = () => {
      for (const [, chunk] of getWebpackChunks()) {
        injectIntoChunk(chunk);
        if (resolved) {
          return;
        }
      }
  ...
  const isCorePlayerInstance = (value) => {
    return (
      value != null &&
      typeof value === "object" &&
      ...
      (
        ...
        value.videoTracks != null ||
        value.player?._mediaController != null
      )
    );
  };

  let activePreviewPlayerModule;
  const getActivePreviewPlayerModule = async () => {
    if (activePreviewPlayerModule !== undefined) {
      return activePreviewPlayerModule;
    }
    const corePlayer = await Promise.race([
      getCorePlayer(),
      new Promise((resolve) => setTimeout(resolve, 800)),
    ]);
    const CorePlayer = corePlayer?.constructor;
    const LiveProvider = corePlayer?.srcObject?.constructor;
    if (
      typeof CorePlayer === "function" &&
      typeof LiveProvider?.fromJSON === "function"
    ) {
      activePreviewPlayerModule = { CorePlayer, LiveProvider };
    }
    return activePreviewPlayerModule;
  };
  ...
  const showPreview = async (href, node, tooltip) => {
    ...
    let Player = await getActivePreviewPlayerModule();
    if (Player == null) {
      try {
        const req = await getWebpackRequire;
        Player = typeof req === "function" ? req(49588) : null;
      } catch { }
    }
    ...
    if (offset) {
      try {
        const topOffset = await findReactState(
          layoutWrap,
          (state) => state.length === 3 && typeof state[0] === "number"
        );
  ...
  const getCorePlayer = async () => {
    ...
    const player = await findReactState(node, (s) => {
      return (
        s?._corePlayer != null ||
        isCorePlayerInstance(s) ||
        findNestedValue(s, isCorePlayerInstance) != null
      );
    });
    return (
      player?._corePlayer ||
      (isCorePlayerInstance(player) ? player : null) ||
      findNestedValue(player, isCorePlayerInstance)
    );
  };
